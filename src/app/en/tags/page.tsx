  ...
  async function fetchTags(url: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(result.error || "Error fetching tags.");
      }
    } catch (error) {
    ...
    <main className="min-h-screen bg-[#0a0a0f] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-12 sm:py-20">
        ...
        <header className="flex flex-col items-center text-center mb-16">
          <a href="/en" className="mb-6 flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/5 border border-white/10 hover:border-red-500/30 transition-colors">
          ...
          <h1 className="text-4xl sm:text-6xl font-black text-white mb-4 tracking-tight leading-tight">
            YouTube <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-red-600">Tag Extractor</span>
          </h1>
          <p className="text-white/40 text-lg max-w-2xl">
            Discover hidden keywords from any video to boost your SEO performance.
          </p>
        </header>
        ...
        <div className="max-w-2xl mx-auto mb-16">
          <div className="p-2 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-md shadow-2xl">
            <InputForm lang="en" onResult={(id) => fetchTags(`https://youtube.com/watch?v=${id}`)} />
          </div>
          <div className="mt-4 flex justify-center gap-6">
            <a href="/en" className="text-white/30 hover:text-white text-xs transition-colors underline underline-offset-4 decoration-white/10">Download Thumbnails</a>
            <span className="text-white/10">|</span>
            <span className="text-red-500 text-xs font-semibold">Tag Extractor</span>
          </div>
        </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 p-6 rounded-3xl bg-white/5 border border-white/10 mb-8">
              ...
              <div className="flex-1 min-w-0 text-center md:text-left">
                <p className="text-red-500 text-[10px] font-bold uppercase tracking-widest mb-1">Video Identified</p>
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-white font-bold text-lg">Found Tags ({data.tags.length})</h3>
                <span className="text-white/20 text-[10px] uppercase tracking-tighter">Click a tag to copy</span>
              </div>
              ...
              {data.tags.length > 0 ? (
                ...
                <div className="text-center py-10">
                  <p className="text-white/40 italic">No tags found for this video.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <footer className="mt-32 pt-8 border-t border-white/5 text-center text-white/20 text-[10px] uppercase tracking-widest">
          © 2026 YouTube Grabber · Professional Tools for YouTubers
        </footer>
      </div>
    </main>
  );
